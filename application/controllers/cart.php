...
class Cart extends MY_Controller{
	...
	//$_POST['location'] = "asda"; //<- uncomment this out , to see the result if the location is tampered 
	$specific_shipping_details = array();
	$shipping_details = $this->product_model->getShipmentInformation($id);
	foreach($shipping_details as $row){
		    $count =0;
	    if(in_array($this->input->post('location'),$row)){ //get all the data with same location inputed by the user in the array
		for($ctr = 0 ;sizeof($qty[0]['product_attribute_ids']) > $ctr ;$ctr++){
		    for($ctr_child = 0 ;sizeof($row['product_attribute_ids']) > $ctr_child ;$ctr_child++){
			if(in_array($qty[0]['product_attribute_ids'][$ctr]['id'],$row['product_attribute_ids'][$ctr_child])){
			    $count++;
			}
			if($count == sizeof($qty[0]['product_attribute_ids'])){
			    $specific_shipping_details = $row; // this
			}
		    }  
		} 
	    }
	    ...
	    'shipping_details' => $specific_shipping_details
            );
