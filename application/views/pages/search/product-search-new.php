...
<section class="bg-search-section-main color-default ">
    <div class="container">
        <?php if($productCount > 0 || $userCount > 0): ?>
            <div class="row">
                ...
                <div class="col-md-9">
                    <div class="tab-head-wrapper">
                        <ul class="list-inline list-unstyled section-title-tab tab-head-container">
                            <li>
                                <a src="search-product-list" data-segment="product" class="<?=$productTab ? 'active' : ''; ?>">
                                    <span class="tab-title">Products</span> <span class="tab-title-count"><?=$productCount; ?></span>
                                </a>
                            </li>
                            <li>
                                <a src="search-seller-list" data-segment="seller" class="<?=$productTab ? '' : 'active'; ?>">
                                    <span class="tab-title">Sellers</span> <span class="tab-title-count"><?=$userCount;?></span>
                                </a>
                            </li>
                        </ul>
                    </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <h3 class="no-search-resul-title">
                    Sorry, your search for <strong><?=html_escape($string); ?></strong> did not match any product or seller.
                </h3>
            </div>
        </div>
        <br/>
...
<div id="hidden-elements">
    ...
    <?php else: ?>
        <input type="hidden" id="hidden-loadUrl" value="/search/seller/more?<?=$_SERVER['QUERY_STRING']; ?>" />
    <?php endif; ?>
    <input type="hidden" id="hidden-totalPage" value="<?=$totalPage; ?>" />
    <input type="hidden" id="hidden-segment" value="<?=html_escape($segment);?>" />
</div> 
