...
class Version20141208164925 extends AbstractMigration
{
    ...
    {
        $this->addSql('
            ALTER TABLE `easyshop`.`es_order_product_history`
            ...
            REFERENCES `easyshop`.`es_order_product_status` (`id_order_product_status`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,
            ADD CONSTRAINT `fk_es_order_product_history_es_order_product`
            FOREIGN KEY (`order_product_id`)
            REFERENCES `easyshop`.`es_order_product` (`id_order_product`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION;
        ');
    }
