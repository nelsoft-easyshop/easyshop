	...
	protected function _build_message()
	{
		...
		for ($i=0; $i < count($this->_attach_name); $i++)
		{
			...
			if ($this->_attach_disp[$i] == 'inline')
			{
				...
				$f = basename($filename);
				$filename_custom = trim($f);
				
				$cid = 'inl_'.$i;
				$cid1 = 'cid:'.$cid;
				$h .= "Content-ID: <".$cid.">".$this->newline;
				$body = $this->matched_replace_between($body, 'src=3D"', '"', $filename_custom, $cid1, 3);
				$body = $this->matched_replace_between($body, 'href=3D"', '"', $filename_custom, $cid1, 3);
				$body = $this->matched_replace_between($body, 'src="', '"', $filename_custom, $cid1, 3);
				$body = $this->matched_replace_between($body, 'href="', '"', $filename_custom, $cid1, 3);
				$body = $this->matched_replace_between($body, 'url(\'', '\');', $filename_custom, $cid1, 3);
			}
